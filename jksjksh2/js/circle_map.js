 ...
 */
"use strict";
function selectMap2(city,hospitalsLocations,type){
	// 百度地图API功能
	...
	for(var m = 0;m<hospitalsLocations.length;m++){
		for(var n =(total[m]-1);n>=0;n--){
			...
			var a={ b1:dataXs,b2:dataYs};
			matArray.push(a);
			//console.log(a);
		}
	}
	console.log(matArray.length);
	var icon1 = new BMap.Icon('images/icon_c.png', new BMap.Size(10, 10), {
	    anchor: new BMap.Size(20, 10),
	    infoWindowAnchor: new BMap.Size(0, 0)//偏移量
	});
	//从卫生院坐标里取坐标作为marker坐标
	for( var m = 0;m<matArray.length;m++){
		//var circleP =new BMap.Point(parseFloat(matArray[m].b1),parseFloat(matArray[m].b2));
		//console.log(parseFloat(matArray[m].b1));
//		var circleOpts={
//            	fillColor:"#fcf303",
//            	fillOpacity:1,/*越小颜色越淡*/
//            	strokeOpacity:0,
//            	strokeWeight:0.1
//            }   
//		var circ=new BMap.Circle(circleP,150,circleOpts);
		var markerq = new BMap.Marker(new BMap.Point(parseFloat(matArray[m].b1),parseFloat(matArray[m].b2)),{
            icon: icon1
       });
...
//全部的
function selectDateCircle(city,type){
	...
	selectMap2(city,hospitalsLocations,type);	
}
